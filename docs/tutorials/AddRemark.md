...
``` java
import static seedu.HouR.commons.util.CollectionUtil.requireAllNonNull;
//...
public class RemarkCommand extends Command {
    //...
...
Create a `RemarkCommandParser` class in the `seedu.HouR.logic.parser` package. The class must extend the `Parser` interface.
...
Create a new `Remark` in `seedu.HouR.model.person`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Simply add the following to [`seedu.HouR.ui.PersonCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
